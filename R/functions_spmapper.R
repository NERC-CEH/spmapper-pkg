...
spmapper <- function(fppolys, tooldir){

  spcodes <- c("Kittiwake" = "BLKI", "Guillemot" = "COGU", "Razorbill" = "RAZO")

  out <- list() # initialise main output list
  ...
  for(i in 1:length(spcodes)){
    ...
    print(paste("Completed", i , "of", length(spcodes), "species:", spname))
    # add focal species results to main output list
    out[[spname]] <- out_sp
  }

  ## #################################
  ## 12. Output summary for all species

  # Generate summary table from all species individual output list
  out$Summary <- do.call(rbind, lapply(out[c("Kittiwake", "Guillemot", "Razorbill")], function(i) {
    data.frame(
      Species = i$spname,                                             # Species
      Estimate_kg = i$allbirds_prey_est,                              # All birds prey mass
      CI_Lower_kg = as.numeric(quantile(i$allbirds_prey_sim, 0.025)), # Lower CI
      CI_Upper_kg = as.numeric(quantile(i$allbirds_prey_sim, 0.975)), # Upper CI

      fp_percent = round(i$fp_ud_overlap * 100, 2),                          # % prey consumption in footprint/s, 2 d.p.
      fp_Estimate_kg = i$allbirds_prey_fp_est,                               # Footprint prey mass
      fp_CI_Lower_kg = as.numeric(quantile(i$allbirds_prey_fp_sim, 0.025)) , # Footprint prey mass - Lower CI
      fp_CI_Upper_kg = as.numeric(quantile(i$allbirds_prey_fp_sim, 0.975))   # Footprint prey mass - Upper CI
    )}
